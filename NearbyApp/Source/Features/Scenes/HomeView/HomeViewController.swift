...
class HomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        homeViewModel.fetchInitalData(completion: { [weak self] categories in
            guard let self = self else { return }
            self.homeView.updateFilterButton(with: categories) { selectedCategories in
                self.filterPlaces(by: selectedCategories)
            }
        })
